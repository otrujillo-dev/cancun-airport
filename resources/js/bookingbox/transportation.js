...
(function () {    
    window.shuttle = {        
        input_from: document.getElementById('xyz-input-from'),
        input_from_items: document.getElementById('xyz-input-from-items'),
        input_to: document.getElementById('xyz-input-to'),
        input_to_items: document.getElementById('xyz-input-to-items'),        
        ...
        setup: {
            ...
            autocompleteSearch: function(keyword = '', element){
                let size = keyword.length;
                if(size < 3) return false;

                window.shuttle.setup.loader(element);
                
                fetch(`/api/transportation/autocomplete`, {
                    method: 'POST',
                    headers: {
                ...
                for (let key in data) {
                    if (data.hasOwnProperty(key)) {
                        ...
                        itemDiv.addEventListener('click', function() { 
                            window.shuttle.setup.makeEvent(element, data[key]);
                        });
            ...
            makeEvent(element, data = {}){
                if(element.id === "xyz-input-from-items"){
                    window.shuttle.input_from_items.innerHTML = '';
                    window.shuttle.input_from.value = data.name;
                    
                    window.shuttle.setup.items.from.name = data.name;
                    window.shuttle.setup.items.from.lat = data.geo.lat;
                    window.shuttle.setup.items.from.lng = data.geo.lng;
                }

                if(element.id === "xyz-input-to-items"){
                    window.shuttle.input_to_items.innerHTML = '';
                    window.shuttle.input_to.value = data.name;
                    
                    window.shuttle.setup.items.to.name = data.name;
                    window.shuttle.setup.items.to.lat = data.geo.lat;
                    window.shuttle.setup.items.to.lng = data.geo.lng;
                }                
            },
            setDate: function( element ){

                if( element.id === "xyz-input-from-date" ){
                    this.items.from.date = element.value;
                    this.items.to.date = element.value;
                    window.shuttle.input_to_date.value = element.value;
                }

                if( element.id === "xyz-input-to-date" ){
                    this.items.to.date = element.value;

                    if( this.serviceType === "RT" ){
                        if( element.value < window.shuttle.input_from_date.value ){                            
                            this.items.to.date = window.shuttle.input_from_date.value;
                            window.shuttle.input_to_date.value = window.shuttle.input_from_date.value;
                        }                                
                    }
                }
            ...
            setServiceType: function(element){
                ...
                if(element.value == "RT"){
                    const returnInput = document.querySelector("#xyz-input-to-date + input");
                    if(returnInput){
            ...
            setError: function(text){
                window.shuttle.errors.textContent = text;                
            },
            getTranslate: function(item){
                return (window.shuttle.translation[window.shuttle.setup.lang][item]) ? window.shuttle.translation[window.shuttle.setup.lang][item] : 'Translate not found';
            },
            validate: function(){
                window.shuttle.errors.textContent = '';
                
                //Validate from data
                if(!window.shuttle.setup.items.from.name || window.shuttle.setup.items.from.name.trim() === '' || !window.shuttle.setup.items.from.lat || window.shuttle.setup.items.from.lat.toString().trim() === '' || !window.shuttle.setup.items.from.lng || window.shuttle.setup.items.from.lng.toString().trim() === ''){
                    this.setError( this.getTranslate("quote.from.input") );
                    return false;
                }

                //Validate to data
                if(!window.shuttle.setup.items.to.name || window.shuttle.setup.items.to.name.trim() === '' || !window.shuttle.setup.items.to.lat || window.shuttle.setup.items.to.lat.toString().trim() === '' || !window.shuttle.setup.items.to.lng || window.shuttle.setup.items.to.lng.toString().trim() === ''){
                    this.setError( this.getTranslate("quote.to.input") );
                    return false;
                }

                //Validate date data
                if(!window.shuttle.setup.items.from.date || window.shuttle.setup.items.from.date.toString().trim() === ''){
                    this.setError( this.getTranslate("quote.from.date") );
                    return false;
                }

                if(window.shuttle.setup.serviceType === "RT"){
                    //Validate date data
                    if(!window.shuttle.setup.items.to.date || window.shuttle.setup.items.to.date.toString().trim() === ''){
                        this.setError( this.getTranslate("quote.to.date") );                        
                        return false;
                    }
                }

                var data = {
                    type: window.shuttle.setup.serviceType,
                    currency: window.shuttle.setup.currency,
                    language: window.shuttle.setup.lang,
                    passengers: window.shuttle.setup.passengers,
                    from: {
                        name: window.shuttle.setup.items.from.name,
                        lat: window.shuttle.setup.items.from.lat,
                        lng: window.shuttle.setup.items.from.lng,
                        date: window.shuttle.setup.items.from.date,
                        time: window.shuttle.setup.items.from.time
                    },
                    to: {
                        name: window.shuttle.setup.items.to.name,
                        lat: window.shuttle.setup.items.to.lat,
                        lng: window.shuttle.setup.items.to.lng,
                        date: window.shuttle.setup.items.to.date,
                        time: window.shuttle.setup.items.to.time
                    }
                };
                
                localStorage.setItem('shuttle_search', JSON.stringify(data));
                
                var form_data = document.createElement('form');
                    form_data.method = 'post';
                    form_data.action = window.shuttle.setup.getTranslate("quote.url");

                let form_data_type = document.createElement('input');
                    form_data_type.type = 'hidden';
                    form_data_type.name = 'type';
                    form_data_type.value = data.type;
                    form_data.appendChild(form_data_type);
                let form_data_currency = document.createElement('input');
                    form_data_currency.type = 'hidden';
                    form_data_currency.name = 'currency';
                    form_data_currency.value = data.currency;
                    form_data.appendChild(form_data_currency);
                let form_data_language = document.createElement('input');
                    form_data_language.type = 'hidden';
                    form_data_language.name = 'language';
                    form_data_language.value = data.language
                    form_data.appendChild(form_data_language);
                let form_data_passengers = document.createElement('input');
                    form_data_passengers.type = 'hidden';
                    form_data_passengers.name = 'passengers';
                    form_data_passengers.value = data.passengers;
                    form_data.appendChild(form_data_passengers);

                let form_data_from = document.createElement('input');
                    form_data_from.type = 'hidden';
                    form_data_from.name = 'from';
                    form_data_from.value = data.from.name;
                    form_data.appendChild(form_data_from);
                let form_data_from_lat = document.createElement('input');
                    form_data_from_lat.type = 'hidden';
                    form_data_from_lat.name = 'from_latitude';
                    form_data_from_lat.value = data.from.lat;
                    form_data.appendChild(form_data_from_lat);
                let form_data_from_lng = document.createElement('input');
                    form_data_from_lng.type = 'hidden';
                    form_data_from_lng.name = 'from_longitude';
                    form_data_from_lng.value = data.from.lng;
                    form_data.appendChild(form_data_from_lng);
                let form_data_from_pickupDate = document.createElement('input');
                    form_data_from_pickupDate.type = 'hidden';
                    form_data_from_pickupDate.name = 'from_pickup_date';
                    form_data_from_pickupDate.value = data.from.date + " " + data.from.time;
                    form_data.appendChild(form_data_from_pickupDate);

                let form_data_to = document.createElement('input');
                    form_data_to.type = 'hidden';
                    form_data_to.name = 'to';
                    form_data_to.value = data.to.name;
                    form_data.appendChild(form_data_to);
                let form_data_to_lat = document.createElement('input');
                    form_data_to_lat.type = 'hidden';
                    form_data_to_lat.name = 'to_latitude';
                    form_data_to_lat.value = data.to.lat;
                    form_data.appendChild(form_data_to_lat);
                let form_data_to_lng = document.createElement('input');
                    form_data_to_lng.type = 'hidden';
                    form_data_to_lng.name = 'to_longitude';
                    form_data_to_lng.value = data.to.lng;
                    form_data.appendChild(form_data_to_lng);
                
                if(this.serviceType === "RT"){
                    let form_data_to_pickupDate = document.createElement('input');
                        form_data_to_pickupDate.type = 'hidden';
                        form_data_to_pickupDate.name = 'to_pickup_date';
                        form_data_to_pickupDate.value = data.to.date + " " + data.to.time;
                        form_data.appendChild(form_data_to_pickupDate);                
                }
                
                let form_data_crfs = document.createElement('input');
                    form_data_crfs.type = 'hidden';
            ...
            load: function(){
                var data = JSON.parse(localStorage.getItem('shuttle_search'));
                    window.shuttle.setup.lang = document.documentElement.getAttribute('lang');

                if(data){

                    window.shuttle.service_type.forEach(input => {                        
                        if (data.type === "OW" && input.value === "OW") {
                            input.checked = true;
                            window.shuttle.setup.setServiceType(input);
                        }
                        if (data.type === "RT" && input.value === "RT") {
                            input.checked = true;
                            window.shuttle.setup.setServiceType(input);
                        }
                    });
                    ...
                    window.shuttle.setup.items.from.name = window.shuttle.setup.getTranslate("place.name");
                    window.shuttle.setup.items.from.lat = window.shuttle.setup.getTranslate("place.lat")
                    window.shuttle.setup.items.from.lng = window.shuttle.setup.getTranslate("place.lng")
                }
            },
    ...
    // Autocomplete FROM
    window.shuttle.input_from.addEventListener('keydown', window.shuttle.setup.setDelay(function (e) {
        window.shuttle.setup.autocompleteSearch(e.target.value, window.shuttle.input_from_items);        
    }, 700));
    window.shuttle.input_from.addEventListener('focus', (e) => {
        e.target.select();
        window.shuttle.setup.autocompleteSearch(e.target.value, window.shuttle.input_from_items);        
    });

    //Autocomplete TO
    window.shuttle.input_to.addEventListener('keydown', window.shuttle.setup.setDelay(function (e) {
        window.shuttle.setup.autocompleteSearch(e.target.value, window.shuttle.input_to_items);        
    }, 700));
    window.shuttle.input_to.addEventListener('focus', (e) => {
        e.target.select();
        window.shuttle.setup.autocompleteSearch(e.target.value, window.shuttle.input_to_items);        
    });

    // Cierra el div cuando se hace clic fuera
    document.addEventListener('click', (event) => {
        const input_from = window.shuttle.input_from;
        const input_to = window.shuttle.input_to;
        const autocompleteFromDiv = document.getElementById('xyz-input-from-items');
        const autocompleteToDiv = document.getElementById('xyz-input-to-items');

        if (!input_from.contains(event.target) && !autocompleteFromDiv.contains(event.target)) {
            autocompleteFromDiv.innerHTML = '';
        }
        
        if (!input_to.contains(event.target) && !autocompleteToDiv.contains(event.target)) {
            autocompleteToDiv.innerHTML = '';
        }
    });

    window.shuttle.service_type.forEach(function (radio) {
        radio.addEventListener("change", function () {
            if (this.checked) {
                window.shuttle.setup.setServiceType(this);
            }
        });
    });
    ...
    // Book now btn
    window.shuttle.btn_book_now.addEventListener('click', function(e){
        e.preventDefault();
        window.shuttle.setup.validate();
    });    
